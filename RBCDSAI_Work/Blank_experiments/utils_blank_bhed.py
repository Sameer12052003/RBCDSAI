...
#         # Fix common Hindi-specific quote issues
...
def exp1(hf_model, df_path, hint, pred_file_path,third_option = None,give_third = False, br = False):
    ...
    for i in range(df.shape[0]):
        ...
        if isinstance(df['stereo'][i], str):
            stereo_word = ','.join(eval(df['stereo'][i]))
        else:
            stereo_word = ','.join((df['stereo'][i]))
            
        if isinstance(df['anti_stereo'][i], str):
            anti_stereo_word = ','.join(eval(df['anti_stereo'][i]))
        else:
            anti_stereo_word = ','.join((df['anti_stereo'][i]))
        ...
        # json_string= safe_json_parse(json_string)   
        # print(output)
        # print(json_string)
