...
def pride_debias(observed_probs, prior=None):
    """
    Apply PriDe debiasing to observed probabilities
    Args:
        observed_probs: numpy array of shape (num_options,)
        prior: optional pre-computed prior probabilities
    Returns:
        debiased probabilities
    """
    observed = np.array(observed_probs)
    if prior is None:
        # Add small epsilon to avoid log(0)
        log_probs = np.log(observed + 1e-10)
        prior = np.exp(log_probs) / np.sum(np.exp(log_probs))
...
def biased_response(hint1, hint2, input, examples, probs_prior=None):
    """
